...
{
    public partial class KyThuatGiauTinApp : Form
    {
        private String stPath, ndPath, decPath;
        private const int offset = 54;

        public KyThuatGiauTinApp()
        {
            InitializeComponent();
        }
        ...
        {
            if (txtPath.Text == "")
                MessageBox.Show("Please choose image!", "Notice");
            else if (txtPassword.Text == "")
            {
                MessageBox.Show("Please enter password!", "Notice");
            } else if (txtPassword.Text.Length < 8 )
            {
                MessageBox.Show("Please enter at least 8 characters.", "Notice");
            }
            else if (txtPassword.Text.Contains(" "))
            {
                MessageBox.Show("Password don't have space!", "Notice");
            }
            else if (txtConfirm.Text == "")
            {
                MessageBox.Show("Please confict password!", "Notice");
            }
            
            else if (txtPassword.Text != txtConfirm.Text)
            {
                MessageBox.Show("Please enter the same value again!", "Notice");
            }
            else if (txtMessage.Text == "")
            {
                MessageBox.Show("Please enter message!", "Notice");
            } else
            {
                ...
                saveDialog.Title = "Choose place to save!";
                ...
                if (saveDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            saveDialog.Dispose();
                            MessageBox.Show("Hiding message success!", "Notice");
                        }
                        catch
                        {
                            MessageBox.Show("Không thể ghi đè lên hình ảnh vừa lưu!", "Lỗi hệ thống");
                        }  
                    }
                }
            }
        ...
        {
            OpenFileDialog openFile = new OpenFileDialog();
            openFile.Title = "Choose image to extract!";
        ...
        {   
            if (txtPasswordDec.Text != "" && txtPathDec.Text != "")
            {
                ...
                {
                    MessageBox.Show("This picture is null!", "Notice");
                    inStream.Close();
                    return;
                }
                ...
                {
                    MessageBox.Show("The password wrong!", "Notice");
                }
                else
                {
                    txtResult.Text = resultMess;
                    MessageBox.Show("Extract success!", "Notice");
                }
                inStream.Close();
            }
            else
            {
                if (txtPathDec.Text == "")
                    MessageBox.Show("Please choose image to extract!", "Notice");
                else
                    MessageBox.Show("Please enter password to extract!", "Notice");
            }
        }
        ...
        private void firstTabPage_Click(object sender, EventArgs e)
        {
        ...
        private void pictureBefore_Click(object sender, EventArgs e)
        {
